...
summary_tibble <- joined_tibble %>%
     ...
     summarize(
         negativewords = n(),
         words = word_count_tibble$word,
         ratio = negativewords / words
     ) %>%
     ungroup() %>%
     mutate(current_MA = MA, MA_number = row_number()) %>%
     select(current_MA, MA_number, words, negativewords, ratio)
 summary_tibble
```

tidybook=avec une colonne mot et une colone métaanalyse (unnested token) OK
tableau avec le nombre de mots/métaanalyse OK
semi join 
summarize pour compter le nombre de mots négatifs
calcul du ratio
