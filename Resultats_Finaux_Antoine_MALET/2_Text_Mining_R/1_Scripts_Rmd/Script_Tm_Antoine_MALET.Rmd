...
#                                 "Resultats_Finaux_Antoine_MALET",
#                                 "1_Web_Scraping_Python",
#                                 "b_Output_Py",
#                                 ))
earthworms <- read.csv("/home/sophie/WORK_LOCAL/RECHERCHE/TRAVAUX_DE_RECHERCHE/Makowski/EarthwormsMetanalysis/Resultats_Finaux_Antoine_MALET/1_Web_Scraping_Python/b_Output_Py/data_articles_metaanalyses.csv")
...
à part entière.
   ...
   <i>MA1: "Soil chemistry turned upside down: a meta-analysis of
invasive earthworm effects on soil chemical properties"   MA2:
"Earthworms affect plant growth and resistance against herbivores: A
meta-analysis"    MA3: "The unseen invaders: introduced earthworms as
drivers of change in plant communities in North American forests (a
meta-analysis)"    MA4: "Earthworms increase plant production: a
meta-analysis"</i>   
...
terre sur les sols et les plantes).    <i>MA1: "Soil chemistry turned
upside down: a meta-analysis of invasive earthworm effects on soil
chemical properties"   MA2: "Earthworms affect plant growth and
resistance against herbivores: A meta-analysis"    MA3: "The unseen
invaders: introduced earthworms as drivers of change in plant
communities in North American forests (a meta-analysis)"    MA4:
"Earthworms increase plant production: a meta-analysis"</i>    \#
...
MA2 et 3 ([-10;5] et [-5;10]).  

Le terme *"strong"* est commun à toutes les métaanalyses. Cependant, sa
valence peut grandement dépendre du contexte de son utilisation.   On
peut constater que le mot positif *"rich/enrich"* est retrouvé dans la
plupart de métaanalyses (1,2 et 3).   Le mot *"benefit"* n'est
retrouvé que dans les MA2 et 4.   Le mot *"protect"* n'est retrouvé
que dans la MA3.   

On peut constater qu'il n'y a aucun mot négatif commun à tous les
corpus.   Le mot *"disturb"* n'est retrouvé que dans les MA 1 et
3.   Le mot *"infest"* est seulement présent dans les MA2 et 4.   Le
...
des pertes de caractéristiques.   Le mot *"sever"* n'est retrouvé que
dans les MA3 et 4, ce qui signifie peut-être qu'elles sont plus
"quantitatives" que les deux autres.   On peut enfin constater que le
...
```
   ...
   Comme on peur le voir, dans le corpus de texte, *"domin"* a donc
une valence **positive** et non négative. Dans la suite de cette
analyse, les occurences négatives de ce mot seront donc négligées.   
...
lors d'éventuelles comparaisons entre métaanalyses.  

<u>Les quatre mots positifs les plus importants en contribution sont:
</u>   \* *MA1:* "effect", affect", "abundant","dominate".   \*
*MA2:* "effect", "affect", "response","enhance".   \* *MA3:* "effect",
"rich", "abundant", "affect".   \* *MA4:* "effect",
"product","affect","fertilize".  

Les mots *"effect"* et *"affect"* sont présents dans toutes les
métaanalyses. Leur sens précis, cependant, dépend du contexte.  
...
le sol, en lien avec des problématiques de survie.   <u>MA2: </u> Les
...
positifs que de mots négatifs.   <u>MA3: </u> Les mots *"rich"* et
...
de terre invasifs.    <u>MA4: </u> Les mots *"product"* et
...
"domin" et "floor") en contribution sont: </u>   \* *MA1:* "invasive",
"invade", "loss", "negative".   \* *MA2:* "drought", "infest",
"negative","dead".   \* *MA3:* "invasive", "disturb",
"burn","negative".   \* *MA4:* "infest", "invade",
"sever","negative".   <u>MA2 et 4:</u>On remarque la présence du mot
...
importants.   <u>MA1 et 4:</u>Dans les MA1 et 4, la présence de
...
("pest")** (MA4).   <u>MA1 et 3:</u> Les mots *"invasive"* (1er
...
vers de terre.   La MA2 semble légèrement différente des autres dans
...
effets négatifs sur les plantes.  

Da façon simpliste, il semble donc possible, à partir de ces graphiques,
de proposer une première catégorisation comme suit:   - **MA1/MA3:
...
écosystèmes natifs.**   - **MA2/MA4: Métaanalyses globalement
...
risque de déséquilibrer l'écosystème natif.**  
...
```
   ...
   Le tablau ci-dessus présente, pour chaque MA, le rapport mot
...
MA4, MA3. On peut donc écrire, de manière synthétique, que:   \*
**MA1:** 35% des mots connotés sont positifs, 65% sont négatifs.   \*
**MA2:** 21% des mots connotés sont positifs, 79% sont négatifs.   \*
**MA3:** 44% des mots connotés sont positifs, 56% sont négatifs.   \*
**MA4:** 41% des mots connotés sont positifs, 59% sont négatifs.   Ces
...
apparent paradoxe.   Cependant, le ratio de mots négatifs est dans
tous les cas \> 50%, ce qui semble indiquer que la vision du vers de
terre présentée dans ces articles est majoritairement négative.  
...
# Approche tf-idf :
   ...
   **Définitions:**    *tf*: term frequency.   *idf*: inverse
...
des termes rares, en jouant sur la pondération.   *tf-idf*: le produit
...
## Loi de Zipf:
   ...
   La loi de Zipf, découverte empiriquement par Zipf (1949) pour des
mots d'un corpus anglais, stipule que si *f* est la fréquence d'un mot
dans le corpus et *r* le rang, alors :   
$frequency \propto \frac{1}{rank}$    (la fréquence d'un mot donné est
...
de sentiment":   - **MA1/MA3:** Métaanalyses globalement défavorables
...
espèce invasive perturbant les écosystèmes natifs.   - **MA2/MA4:**
...
invasive qui risque de déséquilibrer l'écosystème natif.  

En prenant en compte le sujet apparent de chaque métaanalyse, on peut
supposer que:   - **MA1/MA3:** Métaanalyses globalement défavorables
aux vers de terre. Si certains bénéfices, pour la santé des sols
notamment, semblent être reconnus, le vers de terre est perçu comme une
...
espèces invasives viennent probablement d'Europe ou d'Asie.   -
...
notamment par les métaux lourds (Cu, Pb).  
...
```
   ...
   D'après les graphiques ci-dessus, on peut constater que:   Les
...
présent dans la MA1 que dans les autres MA.   Les bigrammes *"soil
...
dans les autres MA.   Les bigrammes *"tree species"*, *"native
...
autres MA.   Les bigrammes *"soil organisms"*, *"plant functionnal"*,
...
plus présent dans la MA4 que dans les autres MA.  
...
probablement d'Europe ou d'Asie.   - **MA2/MA4:** Métaanalyses
...
pollution des sols, notamment par les métaux lourds (Cu, Pb).  
...
espèces invasives**.    - **MA2/MA4:** Métaanalyses globalement
favorables au vers de terre, mettant en avant les bienfaits
physiologiques des vers de terre pour la plante et les avantages de ces
interactions pour favoriser la productivité. Cependant, le vers de terre
peut malgré tout rester nuisible, car c'est une espèce invasive qui
...
des plantes dans l'écologie des sols**.  
...
plus **positif** qu'il ne l'est réellement.   Le bigramme *"not
...
```
   ...
   Lorsque l'on inspecte le jeu de données **pour chacune des MA**
avec la fonction R *"facet_grid"*, on remarque que les mots ayant
conduit au plus d'erreurs d'identification dans chaque MA sont:   -
...
positive que les analyses précédentes le laissaient penser.**   -
...
précédentes le laissaient penser.**   - <u>MA3:</u> Pas de mot positif
...
penser**.   - <u>MA4:</u> *"responsible"* (donc, le bigramme "not
...
et européens sont perçus comme des espèces invasives.    -
**MA2/MA4:** Métaanalyses globalement favorables au vers de terre,
mettant en avant les bienfaits physiologiques des vers de terre pour la
plante et les avantages de ces interactions pour favoriser la
productivité. Cependant, le vers de terre peut malgré tout rester
...
l'effet du mot "not" sur le mot suivant.   - **MA2/MA4:** Métaanalyses
globalement favorables au vers de terre, mettant en avant les bienfaits
physiologiques des vers de terre pour la plante et les avantages de ces
interactions pour favoriser la productivité. Cependant, le vers de terre
peut malgré tout rester nuisible, car c'est une espèce invasive qui
...
"not" sur le mot suivant.   - **Faux positifs / faux négatifs:** On
...
```
   ...
   Ce premier réseau de bigrammes (toutes MA confondues) nous montre
que trois mots sont un centre (un noeud avec au moins trois
connections):   - **earthworm:** Connecté avec *"species"*,
*"invasion"* et *"activity"*.   - **soil:** Connecté avec *"mineral"*,
*"microbial"* et *"organism"*.   - **plant:** Connecté avec
*"native"*, *"plant"* et *"community/communities"*.   Pour le cluster
...
```
   ...
   Dans cette nouvelle version du même graphe:  
- Les liens sont **orientés**, afin de savoir quel mot est le premier de chaque
bigramme.
   - L'opacité des flèches est **proportionnelle à la
fréquence d'occurence** du bigramme (haute fréquence/ opacité élevée,
basse fréquence/opacité réduite).  
- Les noeuds sont plus larges et colorés en bleu clair.   
   Les centres retrouvés sont:   
- **plant:** "plant community", "plant growth", "plant communities",
"native plant".  
- **soil:** "mineral soil", "soil microbial" +
<u>"microbial biomass" (forte association)</u>, "soil organisms".  
-**earthworm:** "invasion", "species", "activity".  
D'autres bigrammes notables sont: 
"forest floor", "exotic earthworm", "organic matter",
"leaf litter", "northern hardwood" et "lumbricus terretris".  
...
```
   ...
   Les centres retrouvées sont:   
- **earthworm :** "invasion", "species".  
D'autres bigrammes notables sont: 
"mineral soil" --> "soil microbial" --> "microbial biomass".  
Ou encore:  
...
```
   ...
   Les centres retrouvées sont:   
- **plant :** "species", "growth", "communities".  
...
```
   ...
   Aucun centre n'est retrouvé pour cette métaanalyse.  
On peut par contre retrouver une chaîne de mots reliés entre eux:   
...
```
   Les centres retrouvées sont:   
- **plant :** "species", "growth", "community/communities".  
...
installed.packages()[names(sessionInfo()$otherPkgs), "Version"] = permet de visualiser les packages R non-natifs installés dans une session R.

# Comptage / corrélation avec widyr:

- Visualiser des co-occurrences de mots par section au sein du corpus de texte (dans mon cas, 1 section = 1 MA) = explorer des co-occurences de mots même non-adjacents --> dans chaque MA, quels mots sont retrouvés proches de quels mots?

```{r Setup co-occurrences}
library(dplyr)
library(tidytext)
library(stringr)

MA_section_words <- earthworms %>%
  select(Abstract, MA) %>%
  mutate(section = row_number() %/% 5) %>% # find word correlation within 5 lines / For example, rows 0-9 will have section 0, rows 10-19 will have section 1, and so on.
  filter(section > 0) %>%  # Filter rows where section is greater than 0
  unnest_tokens(word, Abstract) %>%
  mutate(MA = str_replace(MA, "MA", "")) %>% # replace MA1, MA2, MA3, MA4 with 1, 2, 3 and 4.
  filter(!word %in% stop_words$word)
# MA_section_words$section %>% unique()
```

```{r Comptage des paires de mots avec widyr}
library(widyr)

# count words co-occuring within sections
word_pairs <- MA_section_words %>%
  pairwise_count(word, section, sort = TRUE) # Count the number of times each pair of items appear together within a group defined by "feature." (here, 10-lines sections).
word_pairs %>%
  filter(item1 == "earthworm")
```
Le tableau ci-dessus présente les co-occurence de mots pour chacune des sections de 10 lignes définies au préalable, filtré pour que le premier mot soit "earthworm". Dans une section de dix lignes, les trois groupes de mots qui reviennent le plus souvent et commençant par "earthworm" sont donc **earthworm + earthworms** (sans doute la fin d'une phrase et le début d'une autre), **earthworm + effects** et **"earthworm + experiment"**.  

On veut maintenant observer les corrélations entre les mots: nb de fois où ils apparaissent ensemble / nb de fois où ils apparaissent séparément (coefficient Phi, équiv. corrélation de Pearson).  

Le coefficient Phi permet de mesurer le degré (l’intensité) de liaison entre deux variables binaires (codées 0 ou 1). Les variables peuvent donc être soit des variables dichotomiques par nature (ex. genre = H / F) soit des variables que l’on dichotomise soi-même (découper / faire 2 classes à partir d’une variable continue). Plus le coefficent est proche de 0, moins il y a d’associations entre les 2 variables. Si celui-ci est proche de 1 ou -1, cela indique qu’il existe une forte dépendance (positive ou négative). Source: https://www.soft-concept.com/surveymag/definition-fr/definition-coefficient-de-phi.html.

```{r}
# we need to filter for at least relatively common words first
word_cors <- MA_section_words %>%
  group_by(word) %>%
  pairwise_cor(word, section, sort = TRUE, use = "pairwise") %>% # the use = pairwise is necessary to get a good result.
  na.omit()
# word_cors$correlation %>% unique()
# word_cors$correlation %>% table() --> plus précis que "unique".
```
...
```{r écolo1}
word_cors_ecology1= word_cors %>%
  filter(item1 %in% c("earthworm","soil", "plants")) %>%
  group_by(item1) %>%
  slice_max(correlation, n = 5) %>%  # the plot don't know how to manage the ex-aequo during classifying.
  ungroup() %>%
...
```
  ...
  Le calcul des corrélations à l'intérieur de groupes de cinq lignes montre les termes fréquement employés au voisinage des mots choisis pour cette première partie d'analyse (*earthworm/plant/soil*).  
**earthworm:** *"invasion", "forest", "hardwood", "northern"*. On peut en déduire que dans les textes du corpus, les vers de terre sont souvent liés à des invasions dans les forêts de feuillus d'Amérique du nord. On retrouve ici un thème important de la MA3.  
**plants:** *plant", "detritivores", "aphid", "herbivores"*. On peut en déduire que dans les textes du corpus, les plantes sont souvent mentionnées en lien avec les pucerons, les détritivores et les herbivores. On retrouve ici un thème important de la MA2.  
**soil:** *"anecic", "respiration", "american"*. Les vers de terres anéciques sont une classe de vers de terre définie par M. B. Bouché (*Stratégie lombricienne*, 1977), dont l'une des caractéristiques principales était de creuser de profondes galeries dans le sol (1 à 6 mètres de profondeur). On peut donc penser qu'ils participent très fortement à la *respiration* des sols, probablement plus que les deux autres classes (épigés et endogés), qui, par définition, creusent moins profondément. La présence du mot *"american"* sous-entends que les vers de terre anéciques sont particulièrement surveillées en Amérique. On retrouve ici un terme important de la MA1 (la racine "pool" n'apparaît que dans la MA1 dans la mesure de la fréquence des mots).  

```{r écolo2}
word_cors_ecology2= word_cors %>%
  filter(item1 %in% c("ecosystem","invasion","interrelationship")) %>%
  group_by(item1) %>%
  slice_max(correlation, n = 5) %>%  # the plot don't know how to manage the ex-aequo during classifying.
  ungroup() %>%
  mutate(item2 = reorder(item2, correlation)) %>%
...
word_cors_ecology2
```
  Le calcul des corrélations à l'intérieur de groupes de cinq lignes montre les termes fréquement employés au voisinage des mots choisis pour cette seconde partie d'analyse (*ecosystem/interrelationship/invasion*).  
**ecosystem:** *"engineers","lumbricus", "understanding", "indigenous"*. La présence du "engineer" à proximité de "ecosytem" et de "lumbricus" laisse entendre que les textes composant le corpus désignent souvent le vers de terre comme un agent majeur des écosystèmes. Le terme "understanding" laisse entendre que la question est complexes et que les recherches sur le sujet sont encore en cours. "indigenous" évoque l'idée d'une espèce introduite, qui est donc "indigène" pour son écosystème d'acceuil.   
**interrelationship:** *"polluted", "pb", "metals", "harvest", "cycles"*. La présence de ces termes dans le voisinage du mots "interrelations" montre que le corpus parle sans doute de la pollution des sols aux métaux lourd (comme le plomb), et plus largement du monde l'agriculture, de l'agronomie et des récoltes. On retrouve ici un thème abordé dans la MA4.  
**invasion:** *"forest", "usa", "earthworm", "northern"*. On peut en déduire que dans les textes du corpus, les vers de terre sont souvent liés à des invasions dans les forêts du **nord des USA**. On retrouve ici un thème important de la MA3.  

## Origine géographique:

```{r geo}
word_cors_geo= word_cors %>%
  filter(item1 %in% c("european","american","usa")) %>%
  group_by(item1) %>%
  slice_max(correlation, n = 5) %>%  # the plot don't know how to manage the ex-aequo during classifying.
  ungroup() %>%
  mutate(item2 = reorder(item2, correlation)) %>%
...
```
  ...
  Le calcul des corrélations à l'intérieur de groupes de cinq lignes montre les termes fréquement employés au voisinage des mots choisis pour cette troisième partie d'analyse. Les mots ont été choisis spécifiquement pour tenter de grouper les données selon la région géographique.  
En regardant les encadrés "american" et "usa" (les USA étant un pays d'Amérique), on peut remarquer la présence des termes *"previously", "uninvaded", "invasion"* et *"forests"*. Ce corpus semble donc décrire un processus d'invasion dynamique ("previously uninvaded"), même s'il est plutôt étonnant que ces deux mots ne se trouvent pas dans le même subplot. L'encadré "european" nous apprend que ce mot est souvent utilié à proximité de *"north""* et *"america"*, ce qui laisse entendre que les vers de terre invasifs actifs en Amérique sont majoritairement européens.  

Dans les analyses précédentes, on a vu que:  
- **MA1/MA3:** Métaanalyses globalement défavorables aux vers de terre. Si certains bénéfices, pour la santé des sols notamment, semblent être reconnus, le vers de terre est perçu comme une espèce invasive perturbant les écosystèmes natifs.  Les mots “soil”, “forest” et “effect” (MA1) et “canopy” (MA3), donnent une idée des environnements où se sont déroulées les études (sols forestiers, forêt). Les mots “exotic” (MA1) et “eastern” (MA3) semblent montrer que les espèces invasives viennent probablement d’Europe ou d’Asie.  Les bigrammes “forest soils” (MA1) et “tree species” (MA3) confirment que les études s’intéressent aux environnements forestiers. Les bigrammes “earthworm invasion” (MA1) et “earthworm invasions” (MA3) confirment que les vers de terre asiatiques et européens sont perçus comme des espèces invasives. La MA1 étant plus négative que prévu, sachant que la MA3 (plus positive que prévu) ne semble pas compenser totalement cet effet (cf. “words preceded by not”), on en déduit que ces deux métaanalyses sont un peu plus négatives que présenté précedemment, en raison de l’effet du mot “not” sur le mot suivant. Grâce au calcul des corrélations pour la MA3, il est maintenant possible d'affirmer que les territoires envahis sont principalement le nord des USA.  
- **MA2/MA4:** Métaanalyses globalement favorables au vers de terre, mettant en avant les bienfaits physiologiques des vers de terre pour la plante et les avantages de ces interactions pour favoriser la productivité. Cependant, le vers de terre peut malgré tout rester nuisible, car c’est une espèce invasive qui risque de déséquilibrer l’écosystème natif.  La MA2 semble plus se focaliser sur les effets observées sur les plantes en tant que telles (pucerons, harbivores) alors que la 4 semble davantage axée sur la pollution des sols, notament par les métaux lourds (Cu, Pb).  Le bigramme “soil organisms” est le plus représenté dans les deux métaanalyses, reforçant encore le lien thématique (écologie des sols) commun aux MA2 et 4. Par ailleurs, de nombreuses références aux plantes (“plant responses”, “plant mediated” et “root herbivore” pour la MA2 et “plant functionnal”, “dry weight” et “diplocardia spp” pour la MA4) montrent que les deux MA s’intéressent spécifiquement au rôle des plantes dans l’écologie des sols.  La MA2 et la MA4 sont plus positive que prévu (cf. “words preceded by not”), en raison de l’effet du mot “not” sur le mot suivant.  

Grâce au calcul des corrélations sur les groupes de 5 lignes, on peut maintenant rajouter que:


**MA1/MA3:** Métaanalyses globalement défavorables aux vers de terre. Si certains bénéfices, pour la santé des sols notamment, semblent être reconnus, le vers de terre est perçu comme une espèce invasive perturbant les écosystèmes natifs.


- Les mots “soil”, “forest” et “effect” (MA1) et “canopy” (MA3), donnent une idée des environnements où se sont déroulées les études (sols forestiers, forêt). Les mots “exotic” (MA1) et “eastern” (MA3) semblent montrer que les espèces invasives viennent majoritairement d'Europe (voir "Corrélations: origine géographique").


- Les bigrammes “forest soils” (MA1) et “tree species” (MA3) confirment que les études s’intéressent aux environnements forestiers. Les bigrammes “earthworm invasion” (MA1) et “earthworm invasions” (MA3) confirment que les vers de terre asiatiques et européens sont perçus comme des espèces invasives.

- La MA1 et la MA3 sont moins positives que prévu (cf. “words preceded by not”), en raison de l’effet du mot “not” sur le mot suivant.  

**MA2/MA4:** Métaanalyses globalement favorables au vers de terre, mettant en avant les bienfaits physiologiques des vers de terre pour la plante et les avantages de ces interactions pour favoriser la productivité. Cependant, le vers de terre peut malgré tout rester nuisible, car c’est une espèce invasive qui risque de déséquilibrer l’écosystème natif.


- La MA2 semble plus se focaliser sur les effets observées sur les plantes en tant que telles (pucerons, harbivores) alors que la 4 semble davantage axée sur la pollution des sols, notamment par les métaux lourds (Cu, Pb). Les analyses de la partie "Corrélations: écologie" confirment que certains articles du corpus étudient bel et bien les relations entre les récoltes, les cycles naturels, et la pollution aux métaux lourds (principalement Pb). 


- Le bigramme “soil organisms” est le plus représenté dans les deux métaanalyses, reforçant encore le lien thématique (écologie des sols) commun aux MA2 et 4. Par ailleurs, de nombreuses références aux plantes (“plant responses”, “plant mediated” et “root herbivore” pour la MA2 et “plant functionnal”, “dry weight” et “diplocardia spp” pour la MA4) montrent que les deux MA s’intéressent spécifiquement au rôle des plantes dans l’écologie des sols.

- La MA2 et la MA4 sont plus positive que prévu (cf. “words preceded by not”), en raison de l’effet du mot “not” sur le mot suivant.  

## Graphe des corrélations:

Dans cette partie de l'analyse, les corrélations entre les mots seront représentées par groupe de dix lignes. Pour des questions techniques liés à la visualisation des données, on a imposé le choix du mot central et gardé que les cent premières lignes du tableau des corrélations.

```{r ggraph des corrélations ew}
library(dplyr)
library(ggraph)
...
MA_section_words <- earthworms %>%
  select(Abstract, MA) %>%
  mutate(section = row_number() %/% 10) %>% # find word correlation within 5 lines / For example, rows 0-9 will have section 0, rows 10-19 will have section 1, and so on.
  filter(section > 0) %>%  # Filter rows where section is greater than 0
  unnest_tokens(word, Abstract) %>%
  mutate(MA = str_replace(MA, "MA", "")) %>% # replace MA1, MA2, MA3, MA4 with 1, 2, 3 and 4.
  filter(!word %in% stop_words$word)
...
Pour le premier graphe des corrélations ci-dessus, chaque terme est un noeud, et chaque connection est proportionnelle au dégré de corrélation. On a retenu le terme **"earthworm"** comme premier terme. On peut observer qu'il est fortement connecté avec *"invasion", "dynamics", "northern', "hardwood", "forests"* et *"abundance"*. Il est aussi corrélé avec *"alter", "invasive", "uninvaded", "losses", "american", "european", "exotic", "nonnative"*, et *"soil"*. Enfin, on peut aussi repérer un lien fort avec le terme *"driver"*.  
On remarque dans un premier temps que les mots retrouvés dans la partie "bigrammes" sont fortement corrélés ("earthworm invasion", "exotic earthworm", etc.) ce qui est logique puisqu'ils faisaient justement partie du même groupe de mots.  
On retrouve aussi la vision du vers de terre européen comme une espèce "invasive", dont la nature "exotique" dans les sols provoque des pertes et des altérations écosystémiques, notamment dans les forêts du nord des USA. Cependant, l'apparition à cette étape du terme **"driver"** confirme par Text-mining que le ver de terre est bel et bien une **cause** identifiée pour expliquer les symptômes décrits. De la même façon, l'apparition de *"epigeic"*, mais non de *"endogeic"* ou *"anecic"* suggère que les vers de terre de surface sont probablement davantage étudiés que les vers de terre présents en profondeur.

```{r ggraph des corrélations plant}
library(dplyr)
library(ggraph)
...
Pour le deuxième graphe ci-dessus, chaque terme est un noeud, et chaque connection est proportionnelle au dégré de corrélation. On a retenu le terme **"plant"** comme premier terme. On peut observer qu'il est fortement connecté avec *"plants","detritivores", "affected", "leaves"*, et *"functional"*. Il est aussi corrélé avec *"hebivores", "aphid", "interactions", "reproduction"* et *"development"*.  
On remarque dans un premier temps que les mots retrouvés dans la partie "bigrammes" sont fortement corrélés ("plant leaves", "plant functional"), ce qui est logique puisqu'ils faisaient justement partie du même groupe de mots. On peut aussi constater la présence du mot **"role"**, ce qui, dans le contexte des quatre MA, suggère que les vers de terre on une influence sur les rôles fonctionnels de la plante. Cela rapelle le thème de la MA2 (influence des vers de terre sur la physiologie végétale). 

```{r ggraph des corrélations euro}
library(dplyr)
library(ggraph)
...
Pour le troisième graphe ci-dessus, chaque terme est un noeud, et chaque connection est proportionnelle au dégré de corrélation. On a retenu le terme **"european"** comme premier terme, dans le but de détecter une éventuelle différence d'opinion entre l'Europe et l'Amérique du nord.  
On peut observer qu'il est fortement connecté avec *"north"*, *"america"* et *"disappeared"*. Il est aussi corrélé avec *"destruction", "elimination", "native"* et *"rubellus"*. Les trois premiers mots relevés semblent indiquer que les vers de terre Européens sont bel et bien perçus comme des nuisibles en Amérique du nord,  impression renforcée par les termes "destruction", "elimination", "native" et "rubellus". D'autres termes, comme *"nitrification"* ou *"germinate"* donnent une impression plus positive, mais ils sont en minorité.

```{r ggraph des corrélations american}
library(dplyr)
library(ggraph)
...
Pour le troisième graphe ci-dessus, chaque terme est un noeud, et chaque connection est proportionnelle au dégré de corrélation. On a retenu le terme **"america"** comme premier terme, dans le but de détecter une éventuelle différence d'opinion entre l'Europe et l'Amérique du nord. On a choisi le mot "america" car les résultats obtenus semblaient plus intéressants qu'avec "american" (de nombreuses études composant le corpus étant localisées en Amérique du Nord).  Au seuil de 100 mots, le terme *"terrestris"* était retrouvé, mais pas le terme *"lumbricus"*, qui est souvent retrouvé avec, c'est pourquoi cette figure a été produite avec 150 mots et non 100.  
On peut observer qu'il est fortement connecté avec *"north"*, *"forest(s)", "destruction", "invasion"* et *"disappeared"*. Il est aussi corrélé avec *"earthworm", "lumbricus", "terrestris" "ecosytems", "lumbricids"* et *"rubellus"*.  

Les quatre premiers mots relevés semblent indiquer que les vers de terre invasifs sont bel et bien perçus comme des nuisibles en Amérique du nord (en particulier pour les forêts). La présence des termes *"earthworm", "lumbricus", "terrestris, "ecosytems", "lumbricids"* et *"rubellus"* confirme de manière explicite que les "espèces invasives" mentionnées sont très probablement des vers de terre.

# Fin de "Rapport Text-mining Earthworms"

<!-- Le double espace "  " à la fin d'un mots fait pareil que    -->
