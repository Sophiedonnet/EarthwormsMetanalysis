# A script that opens a web browser instance and scrapes data from ResearchGate source code. For some reason, abstract texts were not found.
...
def scrape_researchgate_publications(query: str):
	"""
	Scrapes ResearchGate data.
	
	Args: 
	    Text(str): publication title.
		
	Returns:
	    str: The extracted result (most notably publication date, doi and target url) if found, otherwise 'null'.
	"""
    with sync_playwright() as p:
        ...
        while True:
            ...
            # a loop that uses style marks (css selector) to identify and extract desired data.
